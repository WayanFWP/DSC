  ...
  for (const QString &line : lines) {
    ...
    if (parts.size() >= 2) {
      ...
      if (ok1 && ok2) {
        ...
        y1Data1.append(y1);
        y2Data1.append(y2);
        traindata.addDataPoint(y1, y2, 0);
      }
    }
  }
...
void MainWindow::on_trainButton_clicked() {
  ...
  // Set the progress callback function before training
  MLGait.setProgressCallback(
      [this](int epoch, double loss, double lr, double loss_change) { this->updateTrainingProgress(epoch, loss, lr, loss_change); });
  ...
  MLGait.train(augmented_data, maxEpochs, learningRate, decay);
...
void MainWindow::on_recallButton_clicked() {
  ...
  for (const auto &sample : testData) {
    ...
    // Create output probabilities string
    QString     outputStr;
...
void MainWindow::plotTrainingMetrics() {
    ...
    metricsPlot->legend->setBrush(QColor(255, 255, 255, 150));
