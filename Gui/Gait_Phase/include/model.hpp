...
class MLP {
 ...
 public:
  ...
  // =============== qt purpose ================
  // Define a callback function type for progress updates
  ...
  void
  train(const std::vector<std::pair<std::vector<double>, int>>& dataset, int epochs = 1000, double learning_rate = 0.25, double decay_rate = 0.01) {
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
      ...
      if (epoch % 10 == 0 || epoch == epochs - 1) {
        ...
        if (progress_callback) {
          progress_callback(epoch + 1, avg_loss, lr, loss_change);
        }
        // Check for nearly zero change (convergence)
        if (std::abs(loss_change) < 1e-15 && epoch > 50) {
      ...
      int correct = 0;
      for (const auto& sample : dataset) {
        std::vector<double> output    = predict(sample.first[0], sample.first[1]);
